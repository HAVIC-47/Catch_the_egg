// Catch The Eggs part 6 - + Perks (6) + debuffs (3) + shield + effect timers + magnet/slow/reverse/freeze
// Build: g++ "Catch The Eggs part 6.cpp" -o "Catch The Eggs part 6.exe" -lfreeglut -lopengl32 -lglu32 -lwinmm
#include <windows.h>
#include <GL/glut.h>
#include <math.h>
#include <stdio.h>
...
const float WIND_INT=75.0f, EGG_SPEED=280.0f;
const float SCORE_MULT=1.75f;
const int POOP_PCT=25, BOMB_PCT=10;
...
void drawSky(){
    drawGrad(0,0,(float)winW,(float)winH, 0.14f,0.16f,0.26f, 0.40f,0.42f,0.52f);
}
...
static const float pcol[OT_COUNT][3] = {
    {1,1,1},{0.30f,0.50f,1.00f},{1.00f,0.85f,0.10f},{0.34f,0.17f,0.04f},{0.15f,0.15f,0.15f},
    {0.0f,0.95f,1.0f},{1.0f,0.15f,0.95f},{1.0f,0.25f,0.25f},{1.0f,0.85f,0.05f},
    {0.25f,0.45f,1.0f},{0.22f,0.88f,0.35f},
    {1.0f,0.50f,0.05f},{0.55f,0.12f,0.78f},{0.58f,0.70f,0.82f}
};
...
void applyPickup(ObjType t,float x,float y){
    ...
    if(t<=EGG_GOLD){
        ...
        int pts=(int)(eggPts[t]*mult*SCORE_MULT*dbl); if(pts<1) pts=1;
        score+=pts; combo++;
        sprintf(buf,"+%d",pts);
        spawnFloat(x,y+10,buf,eggCol[t][0],eggCol[t][1],eggCol[t][2]);
        spawnBurst(x,y,eggCol[t][0],eggCol[t][1],eggCol[t][2],eggBurst[t]);
    } else if(t==POOP)        hitBad(10,x,y,0.40f,0.25f,0.05f,14,false);
    else if(t==BOMB)          hitBad(25,x,y,1.0f,0.35f,0.05f,44,true);
    else if(t==PERK_LARGE)  { effT[EFX_LARGE]=10.0f;  spawnFloat(x,y+10,"LARGE!",0,1,1); }
    else if(t==PERK_SLOW)   { effT[EFX_SLOW]=10.0f;   spawnFloat(x,y+10,"SLOW!",1.0f,0.3f,1.0f); }
    else if(t==PERK_EXTRA)  { timeLeft+=10.0f;        spawnFloat(x,y+10,"+10s",1.0f,0.3f,0.3f); }
    else if(t==PERK_DOUBLE) { effT[EFX_DOUBLE]=10.0f; spawnFloat(x,y+10,"2x POINTS",1.0f,0.88f,0.1f); }
    else if(t==PERK_MAGNET) { effT[EFX_MAGNET]=8.0f;  spawnFloat(x,y+10,"MAGNET!",0.3f,0.5f,1.0f); }
    else if(t==PERK_SHIELD) { if(shield<2) shield++; spawnFloat(x,y+10,"SHIELD",0.3f,0.9f,0.3f); }
    else if(t==DEB_SMALL)   applyDeb(EFX_SMALL,5.0f,"SHRINK!",x,y,1.0f,0.5f,0);
    else if(t==DEB_REVERSE) applyDeb(EFX_REVERSE,5.0f,"REVERSE!",x,y,0.55f,0.12f,0.78f);
    else if(t==DEB_FREEZE)  applyDeb(EFX_FREEZE,3.0f,"FREEZE!",x,y,0.55f,0.70f,0.90f);
...
void resetGame(){
    ...
    sticks.push_back((float)winH-490.0f);
    for(int i=0;i<4;i++){
        Chick c;
        c.si=i%3;
...
void update(float dt){
    ...
    for(int i=0;i<EFX_COUNT;i++) if(effT[i]>0){ effT[i]-=dt; if(effT[i]<0) effT[i]=0; }
    bk.w=bk.bw;
    ...
    if(!effOn(EFX_FREEZE)){
        ...
        if(effOn(EFX_REVERSE)){ bool t=L; L=R; R=t; }
        float sp=720.0f*dt;
...
int main(int argc,char** argv){
    ...
    glutCreateWindow("Catch The Eggs - Part 6");
