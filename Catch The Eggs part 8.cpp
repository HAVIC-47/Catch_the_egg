// Catch The Eggs part 8 - + Audio (BGM + SFX) — full game (= Compact.cpp)
// Build: g++ "Catch The Eggs part 8.cpp" -o "Catch The Eggs part 8.exe" -lfreeglut -lopengl32 -lglu32 -lwinmm
#include <windows.h>
#include <mmsystem.h>
#include <GL/glut.h>
#include <math.h>
...
// === 6. Audio (BGM WAV gen + MCI loop, PlaySound SFX) ===
void generateBGM(const char* path) {
    FILE* f=fopen(path,"rb"); if(f){fclose(f); return;}
    f=fopen(path,"wb"); if(!f) return;
    const int sr=22050; const float nd=0.25f;
    const int spn=(int)(sr*nd), nn=32, ns=spn*nn;
    ...
    float mel[32]={261.63f,329.63f,392.00f,329.63f,261.63f,329.63f,392.00f,329.63f,
                   349.23f,440.00f,523.25f,440.00f,349.23f,440.00f,523.25f,440.00f,
                   392.00f,493.88f,587.33f,493.88f,392.00f,493.88f,587.33f,493.88f,
                   261.63f,329.63f,392.00f,329.63f,261.63f,329.63f,392.00f,329.63f};
    float bass[32]={130.81f,130.81f,130.81f,130.81f,130.81f,130.81f,130.81f,130.81f,
                    174.61f,174.61f,174.61f,174.61f,174.61f,174.61f,174.61f,174.61f,
                    196.00f,196.00f,196.00f,196.00f,196.00f,196.00f,196.00f,196.00f,
                    130.81f,130.81f,130.81f,130.81f,130.81f,130.81f,130.81f,130.81f};
    for(int i=0;i<ns;i++){
        int ni=i/spn; float tl=(float)(i%spn)/sr, t=(float)i/sr;
        float att=0.015f, rel=0.06f, env;
        if(tl<att) env=tl/att;
        else if(tl>nd-rel) env=(nd-tl)/rel;
        else env=1.0f;
        float mf=mel[ni], bf=bass[ni];
        float mw=0.35f*sinf(2*PI*mf*t)+0.12f*sinf(4*PI*mf*t);
        float bw=0.30f*sinf(2*PI*bf*t);
        float s=(mw*env+bw*env*0.7f)*0.6f;
        if(s>1) s=1; if(s<-1) s=-1;
        short samp=(short)(s*20000); fwrite(&samp,2,1,f);
    }
    fclose(f);
}
void startBGM(){
    ...
    if(mciSendString(TEXT("open \"catcheggs_bgm.wav\" type waveaudio alias bgm"),NULL,0,NULL)==0){
        mciSendString(TEXT("play bgm repeat"),NULL,0,NULL); bgmOn=true;
    }
}
void playSfx(int k){
...
void mouseClick(int btn,int st,int x,int y){
    ...
    if(gs==MENU){
        ...
        if(s>=0){ curLvl=s; resetGame(); gs=PLAYING; playSfx(4); }
    }
}
void keyboard(unsigned char k,int x,int y){
    ...
    if(gs==MENU){
        ...
        if(k>='1'&&k<='6'){ curLvl=k-'1'; resetGame(); gs=PLAYING; playSfx(4); }
        ...
        else if(k=='r'||k=='R'){ resetGame(); gs=PLAYING; }
        else if(k=='q'||k=='Q') exit(0);
    }
    glutPostRedisplay();
}
...
int main(int argc,char** argv){
    ...
    glutCreateWindow("Catch The Eggs - Part 8");
    glutFullScreen();
    glEnable(GL_LINE_SMOOTH);
    glHint(GL_LINE_SMOOTH_HINT,GL_NICEST);
    glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
    initGame();
    generateBGM("catcheggs_bgm.wav");
    startBGM();
